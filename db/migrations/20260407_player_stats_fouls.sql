-- Añade faltas a player_stats y las propaga en el cierre transaccional.
...
declare
  ...
  v_fouls_committed_col text;
  v_fouls_received_col text;
...
begin
  ...
  if v_stats_count > 0 then
    select
      ...
      case
        when exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'player_stats' and column_name = 'fouls_committed') then 'fouls_committed'
        when exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'player_stats' and column_name = 'fouls') then 'fouls'
        when exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'player_stats' and column_name = 'pf') then 'pf'
        when exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'player_stats' and column_name = 'faltas') then 'faltas'
        when exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'player_stats' and column_name = 'faltas_cometidas') then 'faltas_cometidas'
        else null
      end,
      case
        when exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'player_stats' and column_name = 'fouls_received') then 'fouls_received'
        when exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'player_stats' and column_name = 'fouls_drawn') then 'fouls_drawn'
        when exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'player_stats' and column_name = 'faltas_recibidas') then 'faltas_recibidas'
        else null
      end,
      case
    ...
    into
      ...
      v_assists_col,
      v_turnovers_col,
      v_fouls_committed_col,
      v_fouls_received_col,
      v_efficiency_col;
    ...
    if v_fouls_committed_col is not null then
      v_insert_cols := v_insert_cols || format(',%I', v_fouls_committed_col);
      v_select_cols := v_select_cols || ',coalesce((s->>''fouls_committed'')::integer, 0)';
    end if;

    if v_fouls_received_col is not null then
      v_insert_cols := v_insert_cols || format(',%I', v_fouls_received_col);
      v_select_cols := v_select_cols || ',coalesce((s->>''fouls_received'')::integer, 0)';
    end if;
